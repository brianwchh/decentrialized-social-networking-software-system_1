...
exports.plugin = {
    ...
    wss.on('connection',async function (socket){
        ...
        socket.on('message',async (message) => {
            connections, userList,me = await handleMessage(socket,message,connections,userList,me);
        ...
        socket.on('close', ()=>{
            ...
            for(let i=0; i<userList.length; i++){
                ...
                try {
                    user = userList[i];
                    conn = connections[user.ip+'/'+user.name]; // if does not throw erro, mannually detect if null
                } catch (err) {
                    return;
                }
...
const handleMessage = async function (connection,message, connections,userList, me) {
    ...
    switch (msg.type) {
        case "login" :
            ...
            // if the user name already exist, return login failure
            user_name = msg.from.name;
            user_ip = msg.from.ip;
            if (connections[user_ip+'/'+user_name]) {
                sendTo(connection, {
                    type: "login",
                    success: false,
                    data : "user exist, choose another name"
                });
            } else {
                const name_str = user_ip+'/'+user_name;
                console.log(name_str); 
                connections[user_ip+'/'+user_name] = await connection;
                await userList.push(
                    {
                        ip: user_ip,
                        name : user_name,
                ...
                console.log(userList);
                console.log("number of connection: ",Object.keys(connections).length)
                ...
                for(let i=0; i<userList.length; i++){
                    ...
                    try {
                        user = userList[i];
                        const name_str = user.ip+'/'+user.name;
                        conn = connections[user.ip+'/'+user.name]; // if does not throw erro, mannually detect if null
                    } catch (err) {
                        return;
                    }
    ...
    return connections,userList,me ;
};
