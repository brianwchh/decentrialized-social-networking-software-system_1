...
const {Group_Model} = require('../Model/Group_Model');
 ...
 * @param {*} h 
 */
Group_Handler.getListView = async (req, h)=> { 
    // validate token
    const data = {
        querySet : [],
    ...
    if (isLoginValid == false){
        data.message = '@Group_Handler.js -> getListView, token expire or null, validate before submit in frontend'
    ...
    // console.log('@Group_Handler.getListView token = ', token)

    const redisRes = await req.app.redis.get(token);

    if (redisRes.errorArray != 0){
        console.log('@Group_Handler.js getListView : ', redisRes.errorArray)
        // query redis error 
        ...
        console.log("@Group_Handler.js getListView , token expored,please login")
        // query redis error 
        data.message = "@Group_Handler.js getListView, token expored,please login"
        data.statusCode = httpStatus.BadRequest
        data.error = ['Bad request']
        return h.response(data).code(data.statusCode)
    }
    ...
    // 3. query group item list by user_id 

    const sqlResult = await req.app.db.query(Group_Model.getItemListByUserId(user_id));

    if (sqlResult.errorArray != 0){
        console.log('@Group_Handler.js : getListView', sqlResult.errorArray)
        data.message = '@Group_Handler.js getListView: ' + sqlResult.errorArray[0]
        data.statusCode = httpStatus.BadRequest
        data.error = ['Bad request']
        return h.response(data).code(data.statusCode)
    }
    ...
    data.message = '@Group_Handler.js : getListView , get Contact item list ok' ;
...
/**
 *  insert one row in Group_Model table
 */
Group_Handler.postListView = async (req, h)=> { 
    ...
    if (isLoginValid == false){
        data.message = '@Group_Handler.js -> postListView, token expire or null, validate before submit in frontend'
        data.statusCode = httpStatus.BadRequest
        data.error = ['Bad request']
        return h.response(data).code(data.statusCode)
    }
    ...
    if (formErrorArray.length != 0){
        data.message = '@Group_Handler.js -> Group_Handler.postListView:  ' + formErrorArray[0]
        data.statusCode = httpStatus.BadRequest
        data.error = ['Bad request']
        return h.response(data).code(data.statusCode)
    }
    ...
    const token = req.query.token 
    // console.log('@Group_Handler.postListView token = ', token)

    // 2. get user ID via searching token in redis 
    const redisRes = await req.app.redis.get(token);

    if (redisRes.errorArray != 0){
        console.log('@Group_Handler.js : postListView ', redisRes.errorArray)
        // query redis error 
        data.message = redisRes.errorArray[0]
        data.statusCode = httpStatus.BadRequest
        data.error = ['Bad request']
        ...
        console.log("@Group_Handler.js postListView , token expored,please login")
        // query redis error 
        data.message = "@Group_Handler.js postListView , token expored,please login"
        data.statusCode = httpStatus.BadRequest
        data.error = ['Bad request']
        return h.response(data).code(data.statusCode)
    }
    ...
    // 4. insert one row in table Group_Model 
    const {name,image,webSocketUrl,hisID} = req.payload ;

    const token2AccessHim = `${hisID}-${my_wsuri}-${my_user_name}`
    const tokenAssigned2Him = `${my_user_name}-${webSocketUrl}-${hisID}`

    sqlResult = await req.app.db.query(Group_Model.insertOneRow(  name,
                                                                    image,
                                                                    webSocketUrl,
                                                                    hisID,
                                                                    user_id,
    ...
    if (sqlResult.errorArray != 0){
        console.log('Group_Handler.js : postListView ', sqlResult.errorArray)
        data.message = '@Group_Handler.js postListView: ' + sqlResult.errorArray[0]
        data.statusCode = httpStatus.BadRequest
        data.error = ['Bad request']
        return h.response(data).code(data.statusCode)
    }
    ...
    // 5. return Contact id to frontend 
    data.message = 'one row has been succesfully inserted to table Group_Model' ;
    data.querySet = sqlResult.querySet ;
...
Group_Handler.putListView = async (req, h)=> { 
...
Group_Handler.deleteListView = async (req, h)=> { 
...
exports.Group_Handler = Group_Handler ;